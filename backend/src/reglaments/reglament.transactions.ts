...
export const actualizeCarsTransaction = async (
  carsToUpdate: Car[],
): Promise<void> => {
  console.log(
    `Starting car actualization transaction via pool at ${new Date().toISOString()}`,
  );
  ...
  try {
    ...
    if (carsToUpdate.length > 0) {
      ...
      const actual_mileages = carsToUpdate.map((car) =>
        BigInt(car.actual_mileage),
      );
      const last_actualizations = carsToUpdate.map(
        (car) => car.last_actualization,
      );
      ...
      console.log(
        `Successfully inserted ${insertResult.rowCount} new car records.`,
      );
    } else {
